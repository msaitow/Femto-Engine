...
namespace Femto { namespace Reaktor {
  ...
  // 
  // *********************************************************
  void SQreaktor::binary_decomposition(vector<SQterm> &theTerms, vector<SQterm> &theLTensors)
  {
    ...
    if(theTerms.size() != theLTensors.size()){
      cout << "Binary_Decomposition: Size of argument1 is inconsistent with that of argument2" << endl;
      abort();
    } // End if
    ...
    for(vector<SQterm>::iterator thisTerm = theTerms.begin();thisTerm != theTerms.end();++thisTerm, ++numTerm){
      ...
      SQtensor thisLTensor(theLTensors[numTerm].get_tensors()[0]);

      // Firstly, set all the external indices and link to the thisLTensor
      {
	    ...
	    thisTerm->get_tensors()[num_t].get_indices()[extamp_]->switch_isExt(true);
          else if(is_D4C(thisTerm->get_tensors()[num_t].get_name()))
	    thisTerm->get_tensors()[num_t].get_indices()[extd4c_]->switch_isExt(true);
	...
	vector<SQindex*> indices(thisTerm->get_summedBody());
        for(size_t num_i = 0;num_i < thisLTensor.get_indices().size();++num_i){
          for(vector<SQindex*>::iterator j = indices.begin();j != indices.end();++j)
	    if(thisLTensor.get_indices()[num_i]->get_index() == (*j)->get_index())
              thisLTensor.put_indices(num_i, *j);
	} // End num_i
        if(isBareLHS_) thisLTensor.get_indices()[extamp_]->switch_isExt(true);
      }
      ...
      if(tensors.size() <= 2){
        ...
        temp_bin.push_back(SQbinary(thisTerm->get_numConst(), thisTerm->get_Consts(), thisLTensor, tensors));
	...
	vector<string> newConsts(thisTerm->get_Consts());
	  ...
	  newConsts.push_back(patterns[optimal_num].get_Ltensor().get_name());
        ...
        temp_bin.push_back(SQbinary(thisTerm->get_numConst(), newConsts, thisLTensor, new_ten));
        binaries_.push_back(temp_bin);
	...
	//*-- ??? --*// } // End else
        ...
        //*-- ??? --*// // Retain the consistency between thisLTensor and thisTerm
        ...
        //*-- ??? --*//     if(**i==**j) thisLTensor.put_indices((size_t)(i-ref_indices.begin()), *j);
  	//*-- ??? --*//   } // End i
