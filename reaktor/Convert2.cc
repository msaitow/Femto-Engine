...
namespace Femto { namespace Reaktor {
  ...
  // *********************************************************
  void SQreaktor::makeCPP_bodyType2(SQbinary &bin, string title, ofstream &f, string s, contDecl c, bool isBareLHS)
  {
    ...
    for(vector<string>::iterator t = NameTen.begin();t != NameTen.end();++t, ++count){
      // This flag represents this ``t" is the type2 intermediate or not
      bool isType2Int(true);
      ...
      /*- There is an intermedaite type tensor detected -*/
        ...
        /*- in case of LHS isn't intermediate -*/
        if(!is_Interm(bin.get_Ltensor().get_name())){
          for(size_t num_t = 0;num_t != bin.get_Rtensors().size();++num_t){
            //if(inTerm.get_tensors()[num_t].get_name() == "X" && !X_indices_.size()){
            /*- in case of RHS has intermediate of usual type -*/ 
            if(is_Interm(bin.get_Rtensors()[num_t].get_name()) && !bin.get_Rindices()[num_t]){
              vector<SQindex*> inds(bin.get_Rtensors()[num_t].get_indices());
            ...
            /*- in case of RHS has intermediate of the special type -*/
            else if(is_Interm(bin.get_Rtensors()[num_t].get_name()) && bin.get_Rindices()[num_t]){
	      isType2Int = true; // That's it!
              vector<SQindex*> inds(bin.get_Rtensors()[num_t].get_indices());
              for(vector<SQindex*>::iterator i = inds.begin();i != inds.end();++i){
        ...
        else if(is_Interm(bin.get_Ltensor().get_name()) && bin.get_Lindices()){
	  isType2Int = true; // That's it!
        ...
        /*- Outside of these (LHS is an intermediate of the usual) -*/        
        else {
          vector<SQindex*> inds(bin.get_Ltensor().get_indices());
	  ...
	  if(!isType2Int) FuncBody += ".cptr(), ";
	  else         FuncBody += "_" + title_ + ".cptr(), ";
	} // End if
        else{
          if(!isType2Int) FuncBody += "&" + *t + ", ";
          else         FuncBody += "&" + *t + "_" + title_ + ", ";
	} // End else
      } // End if
      /*- in case of RDM -*/
  ...
  // 
  // *********************************************************
  //void SQreaktor::makeCPP_body2(SQtensor &LTensor, SQterm &inTerm, string title, ofstream &f, string s, contDecl c, 
  void SQreaktor::makeCPP_body2(SQbinary &bin, string title, ofstream &f, string s, contDecl c, bool isBareLHS)
  {
    vector<string> ExtInd  = c["DecInd"];
    vector<string> Consts  = c["DecConst"];
    vector<string> NameTen = c["DecTensor"];
  ...
  // 
  // *********************************************************
  //void SQreaktor::makeCPP_header2(SQtensor &LTensor, SQterm &inTerm, string title, ofstream &f, string s, contDecl c, 
  void SQreaktor::makeCPP_header2(SQbinary &bin, string title, ofstream &f, string s, contDecl c, bool isBareLHS)
  {
    vector<string> ExtInd  = c["DecInd"];
    vector<string> Consts  = c["DecConst"];
    vector<string> NameTen = c["DecTensor"];
  ...
  void SQreaktor::makeF90_interface2(SQbinary &bin, string title, ofstream &f, string s, contDecl c, bool isBareLHS)
  {
    vector<string> ExtInd  = c["DecInd"];
    vector<string> Consts  = c["DecConst"];
    vector<string> NameTen = c["DecTensor"];
    ...
    // Represents intermediate construction is completely separated from that for the resultant tensor
    // due to the priority of loading ERI from Disk.
    ...
    if(NameTen.size()){
      ...
      if(x_flag && !separate_Loops){
        ...
        for(size_t num_i = 0;num_i < bin.get_Ltensor().get_indices().size();++num_i)
          if(!bin.get_Ltensor().get_indices()[num_i]->get_isExt()) ++num;
        ...
        for(size_t num_t = 0;num_t < bin.get_Rtensors().size();++num_t){
          int num = 0;
          for(size_t num_i = 0;num_i < bin.get_Rtensors()[num_t].get_indices().size();++num_i)
            if(!bin.get_Rtensors()[num_t].get_indices()[num_i]->get_isExt()) ++num; 
          if(!num && is_Interm(bin.get_Rtensors()[num_t].get_name())) 
            scalaNames.push_back(bin.get_Rtensors()[num_t].get_name());
        } // End num_t
      } // End if
    ...
    for(vector<SQtensor>::iterator t = tensors.begin();t != tensors.end();++t){
      ...
      if(is_Interm(t->get_name()) && !separate_Loops){
        string SetX("");
	...
	if     (temp.size() == 0) SetX += "sleft = 0";
      ...
      /*- in case of the intermediate of the special type -*/
      else if(is_Interm(t->get_name()) && separate_Loops){
        string SetX("");
    ...
    for(vector<string>::iterator C = Consts.begin();C != Consts.end();++C)
      SetFunc += *C + ", ";
//*OLD*     for(vector<string>::iterator t = NameTen.begin();t != NameTen.end();++t){
//*OLD*       if(find(keys.begin(), keys.end(), *t) != keys.end())
//*OLD*         SetFunc += extTen[*t] + ", ";
//*OLD*       else
//*OLD*         SetFunc += *t + ", ";
//*OLD*     } // End t
...
//*OLD*     for(vector<string>::iterator s = keys.begin();s != keys.end();++s){
//*OLD*       //if(extTen[*s] == "X" || extTen[*s] == "x") continue;
//*OLD*       if(is_Interm(extTen[*s])) continue;
//*OLD*       string SetDealloc("deallocate(" + extTen[*s] + ")\n");
//*OLD*       f << SetDealloc;
//*OLD*     } // End s

    for(map<string, string>::iterator k = extTen.begin();k != extTen.end();++k){
  ...
  // *********************************************************
  void SQreaktor::makeF90_contract2(SQbinary &bin, string title, ofstream &f, string s, contDecl c, bool isBareLHS)
  {
