...
namespace femto {
  ...
  // 
  // *********************************************************
  void SQreaktor::binary_contract(SQtensor &LTensor, SQterm &inTerm, string title, ofstream &f, string s, contDecl c, 
      bool isBareLHS)
  {

    if(inTerm.get_tensors().size() != 2){
    ...
    DecComm8 += "use comm8_module, only : symblock1, symblock2, symblock3, symblock4, symblock5, symblock6\n";
    DecComm8 += "\n";
    if(use_omp_)
    ...
    vector<SQindex*> LTind(LTensor.get_indices());
    ...
    if(!LTind.size() || !L_nd_ind.size()){
      the_tensors.push_back(inTerm.get_tensors()[0]);
      the_tensors.push_back(inTerm.get_tensors()[1]);
    } // End if
    else{
      vector<SQindex*> temp_i(inTerm.get_tensors()[0].get_indices());
    ...
    //vector<SQindex*> I3(LTensor.get_indices());
    ...
    if(the_tensors[0].get_name() == name_h2_){
    ...
    else if(the_tensors[0].get_name() == "X" && !X_indices_.size()){
      for(size_t num_i = 0;num_i < the_tensors[0].get_indices().size();++num_i){
        SQindex* i(the_tensors[0].get_indices()[num_i]);
      ...
      Rindlist.insert(Rindlist.end(), X_indices_.begin(), X_indices_.end());
    } // End if
    else{
    ...
    else if(the_tensors[1].get_name() == "X" && !X_indices_.size()){
      for(size_t num_i = 0;num_i < the_tensors[1].get_indices().size();++num_i){
        SQindex* i(the_tensors[1].get_indices()[num_i]);
        if(!i->get_isExt()) Rindlist.push_back(i);
      } // End num_i
    ...
    if(!use_daxpy){
    // Call dgemm!
    if(LTensor.get_indices().size())
    f << "! Gemm Z1 * Z2 to form Z3\n";
    else
    f << "! Gemm Z1 * Z2 to form " << LTensor << "\n";
    f << "call dgemm('n', 'n', ";
    ...
    for(size_t i = 0;i < inTerm.get_Consts().size();++i){
      if(inTerm.get_Consts()[i] != "") alpha += "*" + inTerm.get_Consts()[i];
    } // End if
    alpha += ", &";
    f << alpha << endl;
    f << "                     Z1_,&"               << endl;
    ...
    f << "                     Z2_,&"               << endl;
    f << "                     " << k_label << ",&" << endl;
    if(Z3inds.size() || LTensor.get_indices().size()){        
    f << "                     0.0d+00,&"           << endl;        
    f << "                     Z3_,&"               << endl;
    }
    else{
    f << "                     1.0d+00,&"           << endl;        
    f << "                     " << LTensor.get_name() << "_,&" << endl;
    }
    f << "                     " << m_label << ")" << endl << endl;        
    } // End if (use_daxpy)
    else{
    ...
    for(vector<SQindex*>::iterator i = Rindlist.begin();i != Rindlist.end();++i){
      if(!(*i)->get_isExt() && !X_indices_.size()){
    ...
    if     (!isBareLHS && LTensor.get_name() != "X"){
      ...
      if(!X_indices_.size()){
        ...
        Lindlist.insert(Lindlist.end(), X_indices_.begin(), X_indices_.end());
      } // End else
    } // End if
    else{
      for(size_t num_i = 0;num_i < LTensor.get_indices().size();++num_i){
