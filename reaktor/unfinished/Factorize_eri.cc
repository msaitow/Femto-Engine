...
namespace femto {
  ...
  void SQreaktor::factorize_eri()
  {
    ...
    CPfile << "extern std::vector<boost::tuple<std::string, double, double> > my_timer;                 " << endl << endl;         
    }
    ...
    CPfile << "    = orz::ct::BareAmpPack(ctinp, symblockinfo, name_of_sigma); // Sigma(a, a', e, e') tensor                   " << endl;                 
    CPfile << "                                                                                                                " << endl;                           
    ...
    CPfile << "  orz::DTensor rdm4_ij_sliced(nocc*nocc*nocc*nocc*nocc*nocc);                                                   " << endl;
    }
    ...
    // Sort each terms and divide into no_eri  and eri_c, eri_a, eri_v .... 
    vector<SQterm> temp;
    vector<vector<SQterm> > allTerms; allTerms.reserve(inTerms_.size());
    allTerms.push_back(temp); // No ERIs
    allTerms.push_back(temp); // ERI_c
    allTerms.push_back(temp); // ERI_a
    allTerms.push_back(temp); // ERI_v

    for(vector<SQterm>::iterator thisTerm = inTerms_.begin();thisTerm != inTerms_.end();++thisTerm){
      size_t num_flag = 4;
      ...
      for(vector<SQtensor>::iterator t = tensors.begin();t != tensors.end();++t){
        ...
        if(t->get_name() == name_h2_) {
          ...
          if     (inds[exth2_]->get_char() == core) num_flag = 1;
          else if(inds[exth2_]->get_char() == act ) num_flag = 2;
          else if(inds[exth2_]->get_char() == virt) num_flag = 3;
	} // End if
        // In case of the effective tensor
        else if(t->get_name().at(0) == 'Y') {
          string Yflag(t->get_name());
          Yflag.erase(Yflag.begin()); //cout << t->get_name().at(0) << endl;
          int Ynum(atoi(Yflag.c_str()));
          if(Yeffs[(size_t)Ynum].get_tensors()[0].get_name() == name_h2_){
            SQindex* i(Yeffs[(size_t)Ynum].get_tensors()[0].get_indices()[exth2_]);
            if     (i->get_char() == core) num_flag = 1;
            else if(i->get_char() == act ) num_flag = 2;
            else if(i->get_char() == virt) num_flag = 3;
	  } // End if          
	} // End if
        // In case of Bareamp
        else if(t->get_name() == name_amp_) {
      ...
      // 4 for terms without any ERIs .... 
      if(!h2f.first && num_flag == 4) allTerms[0].push_back(*thisTerm);
      else                            allTerms[num_flag].push_back(*thisTerm);      
    } // End thisTerm

    // Print all the sorted terms .... 
    vector<string> label;
    label.push_back("* Terns without ERIs .... \n");
    label.push_back("* Terns with ERIs for core .... \n");
    label.push_back("* Terns with ERIs for act  .... \n");
    label.push_back("* Terns with ERIs for virt .... \n");
    int num = 0;
    for(size_t num_type = 0;num_type < allTerms.size();++num_type){
      cout << label[num_type] << endl;
      for(vector<SQterm>::iterator thisTerm = allTerms[num_type].begin();thisTerm != allTerms[num_type].end();++thisTerm, ++num){
	cout << num << " " << *thisTerm << endl;
      } // End thisTerm
    } // End num_type
    cout << endl;
    ...
    cout << label[num_type] << endl;
    if(num_type) {
      SQindex ieri("ieri", (char_state)(num_type-1));
      vector<SQindex*> temp;
      temp.push_back(&ieri);
      temp.push_back(&ieri);
      temp.push_back(&ieri);
      temp.push_back(&ieri);
      CLoop("  ", ieri, CPfile);
      SQtensor eri(name_h2_, temp, h2_symm());
      ReadERI("    ", eri, CPfile);
      CPfile << endl;
    } // End if
    for(vector<SQterm>::iterator thisTerm = allTerms[num_type].begin();thisTerm != allTerms[num_type].end();++thisTerm, ++numTerm){
	...
	for(vector<SQtensor>::iterator t1 = tensors.begin();t1 != tensors.end();++t1){
        ...
        cout << endl;
        cout << "! The optimal choice is ..... " << endl;
      ...
      if(num_type){
        ...
        for(size_t num_t = 0;num_t < patterns[optimal_num].get_tensors().size();++num_t){
          if(patterns[optimal_num].get_tensors()[num_t].get_name() == name_h2_){
            CPfile << "  int s" + patterns[optimal_num].get_tensors()[num_t].get_indices()[exth2_]->get_index() + ";" << endl;
            CPfile << "  int i" + patterns[optimal_num].get_tensors()[num_t].get_indices()[exth2_]->get_index() + ";" << endl;
            find_eri = true;
	  } // End if
	} // End num_t
	...
	for(vector<string>::iterator c = coeffs.begin();c != coeffs.end();++c){
	  if(c->at(0) == 'Y') {
        ...
        if(!find_eri){
          cout << "Loading of ERI must be on the top of the contraction!" << endl;
          abort();
	} // End if
      } // End if
      if(do_timing_)
      ...
      // Evaluate the reduced tensor if this scheme contains one
      {
        ...
        if(Yflag.first){
	      ...
	      if(num_s != Var.size()-1) DecY += ", ";
            ...
            string DecSymm("  orz::DTensor " + Yten.get_name()  + " = orz::ct::sympack_X" + Yname + "(symblockinfo, 0, Y);");
	    //cout << "Y" + Yname << endl;
            CPfile << DecY    << endl;
            CPfile << DecSymm << endl;
	  } // End if
	...
	// Print the calling section of the Fortran body
      ...
      for(vector<SQindex*>::iterator i = O1.begin();i != O1.end();++i){
        ...
        for(vector<SQtensor>::iterator t = theBody.begin();t != theBody.end();++t){
//*OLD*           if(t->get_name() == name_h2_ && (**i) == *(t->get_indices()[exth2_])){
//*OLD*             cout << Indents[LoopCount] << "Read " + t->get_name() + " from GA for " << (*i)->get_index() << endl;
//*OLD*             ReadERI(Indents[LoopCount], *t, CPfile);
//*OLD*             ERICount = LoopCount;
//*OLD*             t_list.push_back(*t);
//*OLD* 	  } // End if
          if(t->get_name() == name_amp_ && (**i) == *(t->get_indices()[extamp_])){
	...
	DecX += ");";
        ...
        DecSymm += Xname + " = orz::ct::sympack_X" + Xname + "(" + "symblockinfo, " + SymmLabel + ", X);";
        CPfile << DecX    << endl;
        CPfile << DecSymm << endl;
...
//*OLD*           SQindex* i = t->get_indices()[exth2_];
//*OLD* 
...
//*OLD*             printVar += Indents[LoopCount] + "for " + i->get_index() + " in " + Label + ":\n";
//*OLD*             CLoop(Indents[LoopCount], *i, CPfile);
//*OLD*             ++LoopCount;
//*OLD*             Declared.push_back(i);  
//*OLD* 	  } // End if
//*OLD* 
//*OLD*           printVar += Indents[LoopCount] + "Read " + t->get_name() + " from GA for " + i->get_index() + "\n";
//*OLD*           ReadERI(Indents[LoopCount], *t, CPfile);
//*OLD*           ERICount = LoopCount;
//*OLD*           t_list.push_back(*t);
//*OLD* 	} // End if
        if(t->get_name() == name_amp_ && find(t_list.begin(), t_list.end(), *t) == t_list.end()){
	...
	if(!LTensor_.get_indices().size()){
	  ...
	  CPfile << "  " << LTensor_.get_name()  << " = sum_" + LTensor_.get_name() << endl;        
        ...
        if(do_timing_){
          CPfile << "  my_timer.push_back(boost::make_tuple(" << "\"" << title_ << "_no" << numTerm << "\", time.elapsed_cputime(), time.elapsed_wallclocktime()));" << endl;
        } // End if
        CPfile << "  }" << endl;
