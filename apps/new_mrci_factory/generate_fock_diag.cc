...
Eclipse::generate_fock_diag()
{
  ...
  int num_ham = 0;
  string name("< Calculation of the diagonal fock matrix starts > \n\n");
  cout << name;
  status += name;

  print_eqns << " ++ Diagonal elements of Fock Hamiltonian ::" << endl << endl;;
...
#ifdef _CODE_GENE  
    // ********* Construct Sigma *************
    string thisname("fock_d_" + names[num_l]);
