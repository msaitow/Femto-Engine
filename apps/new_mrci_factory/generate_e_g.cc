...
Eclipse::generate_e_g()
{
  ...
  int num_ham = 0;
  string name("< Calculation of the Sigma_{aa'}^{ee'} += <Psi0|EL H T0 |Psi0> > \n\n");
  cout << name;
  status += name;

  print_eqns << " ++ Sigma vector of fic-MRCI (e-g) ::" << endl << endl;
  ...
  for(auto Li = EL_indices.begin();Li != EL_indices.end();++Li){
    ...
    ostringstream stm1;
    stm1 << num_ham++;
    string title("* " + stm1.str() + " <" + names[num_l] + "/g>  ");
    ...
    for(auto p = Ps.begin();p != Ps.end();++p){
      for(auto q = Qs.begin();q != Qs.end();++q){
        ...
        Femto::Core::SQterm term1(1.0, coeffT0, ten1);
	...
	for(auto r = Rs.begin();r != Rs.end();++r){
	  for(auto s = Ss.begin();s != Ss.end();++s){
            ...
            Femto::Core::SQterm term1(0.5, coeffT0, ten1);
...
//     }
    ...
    if(names[num_l].find("c") != string::npos) gen.set_guard_core(true);
    if(names[num_l].find("o") != string::npos) gen.set_guard_act(true);
    ...
    if(combined_result2.size()) {
      ...
      print_eqns << " :::: " << names[num_l] << "/g" << " element :::: " << endl;
      print_eqns << "   " << Sig << " += " << endl;
