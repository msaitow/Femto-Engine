...
#include <sci/ctnew2/c_overlap_ccvv.h>                                            
...
orz::ct::BareAmpPack orz::ct::overlap_ccvv(const orz::ct::Input &ctinp,                                    
  ...
  {
  // No.0
  //* O2(w,y,a,c)  <--  (    4.00000000) T2(w,y,a,c) 
  ...
  for(int ic = symblockinfo.psym()(sc,I_V,I_BEGIN);ic <= symblockinfo.psym()(sc,I_V,I_END);++ic){ 
    ...
    T2b = T2.get_amp2(ic);
    FC_FUNC(g_if_overlap_ccvv_no0_x0, G_IF_OVERLAP_CCVV_NO0_X0)
  ...
  {
  // No.1
  //* O2(w,y,a,c)  <--  (   -2.00000000) T2(y,w,a,c) 
  for(int sc = 0;sc < nir;++sc){ 
  for(int ic = symblockinfo.psym()(sc,I_V,I_BEGIN);ic <= symblockinfo.psym()(sc,I_V,I_END);++ic){ 
    O2b = orz::DTensor(retval.namps_iamp()[ic]);
    T2b = T2.get_amp2(ic);
    FC_FUNC(g_if_overlap_ccvv_no0_x1, G_IF_OVERLAP_CCVV_NO0_X1)
      (sc, ic, T2b.cptr(), O2b.cptr(), nir, nsym, psym);
    retval.acc_amp2(ic, O2b);
  }
  }
  ...
  {
  // No.2
  //* O2(w,y,a,c)  <--  (    4.00000000) T2(y,w,c,a) 
  for(int sc = 0;sc < nir;++sc){ 
  for(int ic = symblockinfo.psym()(sc,I_V,I_BEGIN);ic <= symblockinfo.psym()(sc,I_V,I_END);++ic){ 
    O2b = orz::DTensor(retval.namps_iamp()[ic]);
    ...
    for(int ia = symblockinfo.psym()(sa,I_V,I_BEGIN);ia <= symblockinfo.psym()(sa,I_V,I_END);++ia){ 
      T2b = T2.get_amp2(ia);
      FC_FUNC(g_if_overlap_ccvv_no0_x2, G_IF_OVERLAP_CCVV_NO0_X2)
  ...
  {
  // No.3
  //* O2(w,y,a,c)  <--  (   -2.00000000) T2(w,y,c,a) 
  for(int sc = 0;sc < nir;++sc){ 
  for(int ic = symblockinfo.psym()(sc,I_V,I_BEGIN);ic <= symblockinfo.psym()(sc,I_V,I_END);++ic){ 
    O2b = orz::DTensor(retval.namps_iamp()[ic]);
    for(int sa = 0;sa < nir;++sa){ 
    for(int ia = symblockinfo.psym()(sa,I_V,I_BEGIN);ia <= symblockinfo.psym()(sa,I_V,I_END);++ia){ 
      T2b = T2.get_amp2(ia);
      FC_FUNC(g_if_overlap_ccvv_no0_x3, G_IF_OVERLAP_CCVV_NO0_X3)
        (sa, ia, sc, ic, T2b.cptr(), O2b.cptr(), nir, nsym, psym);
      retval.acc_amp2(ic, O2b);
    }
    }
