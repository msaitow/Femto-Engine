...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no0_x0_type0_noeri &
  (sv0, iv0, W0, nir, nsym, psym, flops)
...
call g_sigma_g_coov_no0_x0_type0_noeri &
  (sv0, iv0, Xcaa, d1, fc1, nir, nsym, psym, flops)

deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no0_x0_type0_noeri
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_coov_no0_x0_type0_noeri &
  (s_v0, i_v0, W0_, D1_, Fc1_, nir, nsym, psym, flops)
...
do i_a1 = psym(I_BEGIN, I_O, s_a1), psym(I_END, I_O, s_a1)
do i_a0 = psym(I_BEGIN, I_O, s_a0), psym(I_END, I_O, s_a0)
W0_(s_a0, s_a1, s_c0)%array(i_a0, i_a1, i_c0) = &
    W0_(s_a0, s_a1, s_c0)%array(i_a0, i_a1, i_c0) &
  + Z3_(i_a1, i_a0, i_c0)
end do
end do
...
end subroutine g_sigma_g_coov_no0_x0_type0_noeri
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no0_x1_type0_noeri &
...
call set_symblock_Xcaa(sleft, W0, nir, nsym, psym) ! -> Xcaa (allocate) 
call g_sigma_g_coov_no0_x1_type0_noeri &
  (sv0, iv0, av2_i, Xcaa, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no0_x1_type0_noeri
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_coov_no0_x1_type0_noeri &
...
! S0() += (   -1.00000000) T2(c0,a1,a0,v0) W0(c0,a1,a0,v0) 
do s_c0 = 0, nir-1
do s_a1 = 0, nir-1
do s_a0 = 0, nir-1
if( &
...
end subroutine g_sigma_g_coov_no0_x1_type0_noeri
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no0_x0_type1_eri_v &
  (sv0, iv0, V2, W1, nir, nsym, psym, flops)
...
call g_sigma_g_coov_no0_x0_type1_eri_v &
  (sv0, iv0, h2_i, Xcaa, d2, nir, nsym, psym, flops)

deallocate(h2_i)
deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no0_x0_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_coov_no0_x0_type1_eri_v &
  (s_v0, i_v0, V2_, W1_, D2_, nir, nsym, psym, flops)
...
integer :: s_c0, i_c0, s_a3, i_a3, s_a2, i_a2, s_a1, i_a1, s_a0, i_a0
...
do i_a1 = psym(I_BEGIN, I_O, s_a1), psym(I_END, I_O, s_a1)
do i_a0 = psym(I_BEGIN, I_O, s_a0), psym(I_END, I_O, s_a0)
W1_(s_a0, s_a1, s_c0)%array(i_a0, i_a1, i_c0) = &
    W1_(s_a0, s_a1, s_c0)%array(i_a0, i_a1, i_c0) &
  + Z3_(i_a1, i_a0, i_c0)
end do
end do
...
end subroutine g_sigma_g_coov_no0_x0_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no0_x1_type1_eri_v &
  (sv0, iv0, T2, W1, S0, nir, nsym, psym, flops)
...
call set_symblock_Xcaa(sleft, W1, nir, nsym, psym) ! -> Xcaa (allocate) 
call g_sigma_g_coov_no0_x1_type1_eri_v &
  (sv0, iv0, av2_i, Xcaa, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no0_x1_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_coov_no0_x1_type1_eri_v &
  (s_v0, i_v0, T2_, W1_, S0_, nir, nsym, psym, flops)
...
integer :: s_c0, i_c0, s_a1, i_a1, s_a0, i_a0
! S0() += (   -1.00000000) T2(c0,a1,a0,v0) W1(c0,a1,a0,v0) 
do s_c0 = 0, nir-1
do s_a1 = 0, nir-1
do s_a0 = 0, nir-1
if( &
...
end subroutine g_sigma_g_coov_no0_x1_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no1_x0_type1_eri_v &
...
call g_sigma_g_coov_no1_x0_type1_eri_v &
  (sv0, iv0, h2_i, Xcaa, d2, nir, nsym, psym, flops)

deallocate(h2_i)
deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no1_x0_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_coov_no1_x0_type1_eri_v &
...
integer :: s_a3, i_a3, s_c0, i_c0, s_a2, i_a2, s_a0, i_a0, s_a1, i_a1
! W2(c0,a1,a0,v0) += (    1.00000000) V2(v0,a3,c0,a2) D2(a3,a0,a1,a2) 
do s_a3 = 0, nir-1
do s_c0 = 0, nir-1
do s_a2 = 0, nir-1
do s_a0 = 0, nir-1
...
end subroutine g_sigma_g_coov_no1_x0_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no1_x1_type1_eri_v &
...
call set_symblock_Xcaa(sleft, W2, nir, nsym, psym) ! -> Xcaa (allocate) 
call g_sigma_g_coov_no1_x1_type1_eri_v &
  (sv0, iv0, av2_i, Xcaa, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no1_x1_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_coov_no1_x1_type1_eri_v &
...
! S0() += (   -1.00000000) T2(c0,a0,a1,v0) W2(c0,a1,a0,v0) 
do s_c0 = 0, nir-1
do s_a0 = 0, nir-1
do s_a1 = 0, nir-1
if( &
...
if(psym(I_LENGTH,I_C, s_c0)*psym(I_LENGTH,I_O, s_a0)*psym(I_LENGTH,I_O, s_a1) > 0) then
...
call dgemm('n', 'n', 1,&
                     1,&
                     psym(I_LENGTH,I_C, s_c0)*psym(I_LENGTH,I_O, s_a0)*psym(I_LENGTH,I_O, s_a1),&
                     - 1.00000000d+00, &
                     Z1_,&
                     1,&
                     Z2_,&
...
flops = flops + 1 * &
                1 * &
                psym(I_LENGTH,I_C, s_c0)*psym(I_LENGTH,I_O, s_a0)*psym(I_LENGTH,I_O, s_a1) * 2.0d+00
...
end subroutine g_sigma_g_coov_no1_x1_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no2_x0_type1_eri_v &
...
call g_sigma_g_coov_no2_x0_type1_eri_v &
  (sv0, iv0, h2_i, Xcaa, d1, nir, nsym, psym, flops)

deallocate(h2_i)
deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no2_x0_type1_eri_v
...
!    -- allRDM.second true
!    -- precedence    -1
subroutine g_sigma_g_coov_no2_x0_type1_eri_v &
  (s_v0, i_v0, V2_, W3_, D1_, nir, nsym, psym, flops)
...
do i_a0 = psym(I_BEGIN, I_O, s_a0), psym(I_END, I_O, s_a0)
do i_a1 = psym(I_BEGIN, I_O, s_a1), psym(I_END, I_O, s_a1)
W3_(s_a1, s_a0, s_c0)%array(i_a1, i_a0, i_c0) = &
    W3_(s_a1, s_a0, s_c0)%array(i_a1, i_a0, i_c0) &
  + Z3_(i_c0, i_a1, i_a0)
end do
end do
...
end subroutine g_sigma_g_coov_no2_x0_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no2_x1_type1_eri_v &
  (sv0, iv0, T2, W3, S0, nir, nsym, psym, flops)
...
call set_symblock_Xcaa(sleft, W3, nir, nsym, psym) ! -> Xcaa (allocate) 
call g_sigma_g_coov_no2_x1_type1_eri_v &
  (sv0, iv0, av2_i, Xcaa, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no2_x1_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_coov_no2_x1_type1_eri_v &
  (s_v0, i_v0, T2_, W3_, S0_, nir, nsym, psym, flops)
...
integer :: s_c0, i_c0, s_a0, i_a0, s_a1, i_a1
! S0() += (    2.00000000) T2(c0,a0,a1,v0) W3(c0,a0,a1,v0) 
do s_c0 = 0, nir-1
do s_a0 = 0, nir-1
do s_a1 = 0, nir-1
if( &
...
end subroutine g_sigma_g_coov_no2_x1_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no3_x0_type1_eri_v &
...
call g_sigma_g_coov_no3_x0_type1_eri_v &
  (sv0, iv0, h2_i, Xcaa, d1, nir, nsym, psym, flops)

deallocate(h2_i)
deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no3_x0_type1_eri_v
...
!    -- allRDM.second true
!    -- precedence    -1
subroutine g_sigma_g_coov_no3_x0_type1_eri_v &
  (s_v0, i_v0, V2_, W4_, D1_, nir, nsym, psym, flops)
...
do i_a0 = psym(I_BEGIN, I_O, s_a0), psym(I_END, I_O, s_a0)
do i_a1 = psym(I_BEGIN, I_O, s_a1), psym(I_END, I_O, s_a1)
W4_(s_a1, s_a0, s_c0)%array(i_a1, i_a0, i_c0) = &
    W4_(s_a1, s_a0, s_c0)%array(i_a1, i_a0, i_c0) &
  + Z3_(i_c0, i_a1, i_a0)
end do
end do
...
end subroutine g_sigma_g_coov_no3_x0_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_coov_no3_x1_type1_eri_v &
  (sv0, iv0, T2, W4, S0, nir, nsym, psym, flops)
...
call set_symblock_Xcaa(sleft, W4, nir, nsym, psym) ! -> Xcaa (allocate) 
call g_sigma_g_coov_no3_x1_type1_eri_v &
  (sv0, iv0, av2_i, Xcaa, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xcaa)

end subroutine g_if_sigma_g_coov_no3_x1_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_coov_no3_x1_type1_eri_v &
  (s_v0, i_v0, T2_, W4_, S0_, nir, nsym, psym, flops)
...
integer :: s_c0, i_c0, s_a0, i_a0, s_a1, i_a1
! S0() += (   -1.00000000) T2(c0,a0,a1,v0) W4(c0,a0,a1,v0) 
do s_c0 = 0, nir-1
do s_a0 = 0, nir-1
do s_a1 = 0, nir-1
if( &
...
end subroutine g_sigma_g_coov_no3_x1_type1_eri_v
