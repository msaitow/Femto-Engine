...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no0_x0_type0_noeri &
...
call g_sigma_g_ocov_no0_x0_type0_noeri &
  (sa0, ia0, Xcav, d1, fc1, nir, nsym, psym, flops)

deallocate(Xcav)

end subroutine g_if_sigma_g_ocov_no0_x0_type0_noeri
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no0_x0_type0_noeri &
  (s_a0, i_a0, W0_, D1_, Fc1_, nir, nsym, psym, flops)
...
integer :: s_a1, i_a1, s_v0, i_v0, s_c0, i_c0
! W0(c0,a1,a0,v0) += (    1.00000000) D1(a1,a0) Fc1(v0,c0) 
do s_a1 = 0, nir-1
do s_v0 = 0, nir-1
do s_c0 = 0, nir-1
if( &
IEOR(s_c0,s_a1) == IEOR(s_a0,s_v0) .and. & 
...
do i_a1 = psym(I_BEGIN, I_O, s_a1), psym(I_END, I_O, s_a1)
do i_v0 = psym(I_BEGIN, I_V, s_v0), psym(I_END, I_V, s_v0)
W0_(s_v0, s_a1, s_c0)%array(i_v0, i_a1, i_c0) = &
    W0_(s_v0, s_a1, s_c0)%array(i_v0, i_a1, i_c0) &
  + Z3_(i_v0, i_c0, i_a1)
end do
end do
...
end subroutine g_sigma_g_ocov_no0_x0_type0_noeri
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no0_x1_type0_noeri &
...
call set_symblock_Xcav(sleft, W0, nir, nsym, psym) ! -> Xcav (allocate) 
call g_sigma_g_ocov_no0_x1_type0_noeri &
  (sa0, ia0, av2_i, Xcav, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xcav)

end subroutine g_if_sigma_g_ocov_no0_x1_type0_noeri
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no0_x1_type0_noeri &
...
integer :: s_c0, i_c0, s_a1, i_a1, s_v0, i_v0
! S0() += (    2.00000000) T2(c0,a1,v0,a0) W0(c0,a1,a0,v0) 
do s_c0 = 0, nir-1
do s_a1 = 0, nir-1
do s_v0 = 0, nir-1
if( &
IEOR(s_c0,s_a1) == IEOR(s_v0,s_a0) .and. &
IEOR(s_c0,s_a1) == IEOR(s_a0,s_v0)) then

if(psym(I_LENGTH,I_C, s_c0)*psym(I_LENGTH,I_O, s_a1)*psym(I_LENGTH,I_V, s_v0) > 0) then
...
call dgemm('n', 'n', 1,&
                     1,&
                     psym(I_LENGTH,I_C, s_c0)*psym(I_LENGTH,I_O, s_a1)*psym(I_LENGTH,I_V, s_v0),&
                     2.00000000d+00, &
                     Z1_,&
                     1,&
                     Z2_,&
...
flops = flops + 1 * &
                1 * &
                psym(I_LENGTH,I_C, s_c0)*psym(I_LENGTH,I_O, s_a1)*psym(I_LENGTH,I_V, s_v0) * 2.0d+00
...
end subroutine g_sigma_g_ocov_no0_x1_type0_noeri
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no0_x0_type1_eri_v &
...
call g_sigma_g_ocov_no0_x0_type1_eri_v &
  (sa0, ia0, sv0, iv0, h2_i, Xca, d2, nir, nsym, psym, flops)

deallocate(h2_i)
deallocate(Xca)

end subroutine g_if_sigma_g_ocov_no0_x0_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no0_x0_type1_eri_v &
  (s_a0, i_a0, s_v0, i_v0, V2_, W1_, D2_, nir, nsym, psym, flops)
...
integer :: s_c0, i_c0, s_a3, i_a3, s_a2, i_a2, s_a1, i_a1
! W1(c0,a1,a0,v0) += (    1.00000000) V2(v0,c0,a3,a2) D2(a3,a2,a1,a0) 
do s_c0 = 0, nir-1
do s_a3 = 0, nir-1
do s_a2 = 0, nir-1
do s_a1 = 0, nir-1
if( &
IEOR(s_c0,s_a1) == IEOR(s_a0,s_v0) .and. & 
IEOR(s_v0,s_c0) == IEOR(s_a3,s_a2) .and. &
...
end subroutine g_sigma_g_ocov_no0_x0_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no0_x1_type1_eri_v &
...
call set_symblock_Xca(sleft, W1, nir, nsym, psym) ! -> Xca (allocate) 
call g_sigma_g_ocov_no0_x1_type1_eri_v &
  (sa0, ia0, sv0, iv0, av2_i, Xca, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xca)

end subroutine g_if_sigma_g_ocov_no0_x1_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no0_x1_type1_eri_v &
...
! S0() += (    2.00000000) T2(c0,a1,v0,a0) W1(c0,a1,a0,v0) 
do s_c0 = 0, nir-1
do s_a1 = 0, nir-1
if( &
IEOR(s_c0,s_a1) == IEOR(s_v0,s_a0) .and. &
...
end subroutine g_sigma_g_ocov_no0_x1_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no1_x0_type1_eri_v &
  (sa1, ia1, sv0, iv0, V2, W2, nir, nsym, psym, flops)

use f_mr_module, only : symblock1, symblock2, symblock3, symblock4, symblock5, symblock6
use f_mr_if_module
implicit none

integer, intent(inout) :: sa1, ia1, sv0, iv0
...
call g_sigma_g_ocov_no1_x0_type1_eri_v &
  (sa1, ia1, sv0, iv0, h2_i, Xca, d2, nir, nsym, psym, flops)

deallocate(h2_i)
deallocate(Xca)

end subroutine g_if_sigma_g_ocov_no1_x0_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no1_x0_type1_eri_v &
  (s_a1, i_a1, s_v0, i_v0, V2_, W2_, D2_, nir, nsym, psym, flops)
...
integer :: s_a3, i_a3, s_c0, i_c0, s_a2, i_a2, s_a0, i_a0
! W2(c0,a1,a0,v0) += (    1.00000000) V2(v0,a3,c0,a2) D2(a3,a2,a1,a0) 
do s_a3 = 0, nir-1
do s_c0 = 0, nir-1
do s_a2 = 0, nir-1
do s_a0 = 0, nir-1
if( &
IEOR(s_c0,s_a1) == IEOR(s_a0,s_v0) .and. & 
IEOR(s_v0,s_a3) == IEOR(s_c0,s_a2) .and. &
IEOR(s_a3,s_a2) == IEOR(s_a1,s_a0)) then
...
end subroutine g_sigma_g_ocov_no1_x0_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no1_x1_type1_eri_v &
  (sa1, ia1, sv0, iv0, T2, W2, S0, nir, nsym, psym, flops)
...
call set_symblock_Xca(sleft, W2, nir, nsym, psym) ! -> Xca (allocate) 
call g_sigma_g_ocov_no1_x1_type1_eri_v &
  (sa1, ia1, sv0, iv0, av2_i, Xca, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xca)

end subroutine g_if_sigma_g_ocov_no1_x1_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no1_x1_type1_eri_v &
  (s_a1, i_a1, s_v0, i_v0, T2_, W2_, S0_, nir, nsym, psym, flops)
...
! S0() += (   -1.00000000) T2(c0,a0,v0,a1) W2(c0,a1,a0,v0) 
do s_c0 = 0, nir-1
do s_a0 = 0, nir-1
if( &
IEOR(s_c0,s_a0) == IEOR(s_v0,s_a1) .and. &
IEOR(s_c0,s_a1) == IEOR(s_a0,s_v0)) then
...
end subroutine g_sigma_g_ocov_no1_x1_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no2_x0_type1_eri_v &
  (sa1, ia1, sv0, iv0, V2, W3, nir, nsym, psym, flops)
...
call g_sigma_g_ocov_no2_x0_type1_eri_v &
  (sa1, ia1, sv0, iv0, h2_i, Xca, d1, nir, nsym, psym, flops)

deallocate(h2_i)
deallocate(Xca)

end subroutine g_if_sigma_g_ocov_no2_x0_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no2_x0_type1_eri_v &
  (s_a1, i_a1, s_v0, i_v0, V2_, W3_, D1_, nir, nsym, psym, flops)
...
! W3(c0,a0,a1,v0) += (    1.00000000) V2(v0,a2,c0,a1) D1(a2,a0) 
do s_a2 = 0, nir-1
do s_c0 = 0, nir-1
do s_a0 = 0, nir-1
if( &
IEOR(s_c0,s_a0) == IEOR(s_a1,s_v0) .and. & 
...
end subroutine g_sigma_g_ocov_no2_x0_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no2_x1_type1_eri_v &
  (sa1, ia1, sv0, iv0, T2, W3, S0, nir, nsym, psym, flops)
...
call set_symblock_Xca(sleft, W3, nir, nsym, psym) ! -> Xca (allocate) 
call g_sigma_g_ocov_no2_x1_type1_eri_v &
  (sa1, ia1, sv0, iv0, av2_i, Xca, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xca)

end subroutine g_if_sigma_g_ocov_no2_x1_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no2_x1_type1_eri_v &
  (s_a1, i_a1, s_v0, i_v0, T2_, W3_, S0_, nir, nsym, psym, flops)
...
integer :: s_c0, i_c0, s_a0, i_a0
! S0() += (   -1.00000000) T2(c0,a0,v0,a1) W3(c0,a0,a1,v0) 
do s_c0 = 0, nir-1
do s_a0 = 0, nir-1
if( &
IEOR(s_c0,s_a0) == IEOR(s_v0,s_a1) .and. &
...
end subroutine g_sigma_g_ocov_no2_x1_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no3_x0_type1_eri_v &
  (sa1, ia1, sv0, iv0, V2, W4, nir, nsym, psym, flops)
...
call g_sigma_g_ocov_no3_x0_type1_eri_v &
  (sa1, ia1, sv0, iv0, h2_i, Xca, d1, nir, nsym, psym, flops)

deallocate(h2_i)
deallocate(Xca)

end subroutine g_if_sigma_g_ocov_no3_x0_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no3_x0_type1_eri_v &
  (s_a1, i_a1, s_v0, i_v0, V2_, W4_, D1_, nir, nsym, psym, flops)
...
integer :: s_c0, i_c0, s_a2, i_a2, s_a0, i_a0
! W4(c0,a0,a1,v0) += (    1.00000000) V2(v0,c0,a2,a1) D1(a2,a0) 
do s_c0 = 0, nir-1
do s_a2 = 0, nir-1
do s_a0 = 0, nir-1
if( &
...
end subroutine g_sigma_g_ocov_no3_x0_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_g_ocov_no3_x1_type1_eri_v &
  (sa1, ia1, sv0, iv0, T2, W4, S0, nir, nsym, psym, flops)
...
call set_symblock_Xca(sleft, W4, nir, nsym, psym) ! -> Xca (allocate) 
call g_sigma_g_ocov_no3_x1_type1_eri_v &
  (sa1, ia1, sv0, iv0, av2_i, Xca, S0, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xca)

end subroutine g_if_sigma_g_ocov_no3_x1_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_g_ocov_no3_x1_type1_eri_v &
  (s_a1, i_a1, s_v0, i_v0, T2_, W4_, S0_, nir, nsym, psym, flops)
...
integer :: s_c0, i_c0, s_a0, i_a0
! S0() += (    2.00000000) T2(c0,a0,v0,a1) W4(c0,a0,a1,v0) 
do s_c0 = 0, nir-1
do s_a0 = 0, nir-1
if( &
IEOR(s_c0,s_a0) == IEOR(s_v0,s_a1) .and. &
...
end subroutine g_sigma_g_ocov_no3_x1_type1_eri_v
