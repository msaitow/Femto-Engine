...
#include <sci/icmr/Femto/elems/c_sigma_g_ccvv.h>                                  
...
double orz::mr::Femto::sigma_g_ccvv(const orz::mr::Input &ctinp,                                                  
...
//-@type(1).contraction(begin)
  ...
  { // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
  // -- No. [    0] -- 
  // |-- [    0] --| S0() += (    4.00000000) T2(c1,c0,v1,v0) V2(v1,c1,v0,c0) 
  int sv1(s_eri);
  int iv1(i_eri);
  ...
  for(int iv0 = symblockinfo.psym()(sv0,I_V,I_BEGIN);iv0 <= symblockinfo.psym()(sv0,I_V,I_END);++iv0){ 
    T2b = T2.get_amp2(iv0);
    FC_FUNC(g_if_sigma_g_ccvv_no0_x0_type1_eri_v,G_IF_SIGMA_G_CCVV_NO0_X0_TYPE1_ERI_V)
      (sv0, iv0, sv1, iv1, T2b.cptr(), V2_sym.cptr(), &S0, nir, nsym, psym, &flops);
  ...
  { // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
  // -- No. [    1] -- 
  // |-- [    0] --| S0() += (   -2.00000000) T2(c1,c0,v1,v0) V2(v1,c0,v0,c1) 
  int sv1(s_eri);
  int iv1(i_eri);
  double flops(0); // Flop count  
  // Pref: 2
  ...
  for(int iv0 = symblockinfo.psym()(sv0,I_V,I_BEGIN);iv0 <= symblockinfo.psym()(sv0,I_V,I_END);++iv0){ 
    T2b = T2.get_amp2(iv0);
    FC_FUNC(g_if_sigma_g_ccvv_no1_x0_type1_eri_v,G_IF_SIGMA_G_CCVV_NO1_X0_TYPE1_ERI_V)
      (sv0, iv0, sv1, iv1, T2b.cptr(), V2_sym.cptr(), &S0, nir, nsym, psym, &flops);
  // --> @[v0, "virtual"] [notNeeded]
  } // End iv0
  } // End sv0
...
//-@D4C.contractions(end)
  ...
  // Do timing!
  orz::mr::Femto::my_timer.push_back(boost::make_tuple("sigma_g_ccvv", time_sigma.elapsed_cputime(), time_sigma.elapsed_wallclocktime()));
