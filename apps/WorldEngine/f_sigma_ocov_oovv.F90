...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_ocov_oovv_no0_x0_type1_eri_c &
...
call g_sigma_ocov_oovv_no0_x0_type1_eri_c &
  (sw, iw, h2_i, Xaaaav, d3, nir, nsym, psym, flops)

deallocate(h2_i)
deallocate(Xaaaav)

end subroutine g_if_sigma_ocov_oovv_no0_x0_type1_eri_c
...
! >> Score :: 111001
! RDM is rotated :: D3(j,i,a1,a3,a0,a2)  >> D3(i,j,a3,a1,a2,a0) 
! summedInd : @[a3, "active"] @[a2, "active"] 
! colInd : @[a1, "active"] @[a0, "active"] @[j, "active"] @[i, "active"] 
subroutine g_sigma_ocov_oovv_no0_x0_type1_eri_c &
...
integer :: s_a3, i_a3, s_v0, i_v0, s_a2, i_a2, s_j, i_j, s_i, i_i
integer :: s_a1, i_a1, s_a0, i_a0
! W0(w,j,a1,a0,i,v0) += (    1.00000000) V2(w,a3,v0,a2) D3(j,i,a1,a3,a0,a2) 
do s_a3 = 0, nir-1
do s_v0 = 0, nir-1
do s_a2 = 0, nir-1
do s_j = 0, nir-1
...
IEOR(IEOR(s_j,s_i),s_a1) == IEOR(IEOR(s_a3,s_a0),s_a2)) then
...
do i_a2 = psym(I_BEGIN, I_O, s_a2), psym(I_END, I_O, s_a2)
do i_a3 = psym(I_BEGIN, I_O, s_a3), psym(I_END, I_O, s_a3)
Z2_(i_a3, i_a2, i_a1, i_a0, i_j, i_i) =  &
  D3_(s_a0, s_a2, s_a1, s_a3, s_j, s_i)%array(i_a0, i_a2, i_a1, i_a3, i_j, i_i)
end do
end do
end do
end do
...
end subroutine g_sigma_ocov_oovv_no0_x0_type1_eri_c
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_ocov_oovv_no0_x1_type1_eri_c &
...
call g_sigma_ocov_oovv_no0_x1_type1_eri_c &
...
end subroutine g_if_sigma_ocov_oovv_no0_x1_type1_eri_c
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_ocov_oovv_no0_x1_type1_eri_c &
...
! S2(i,w,j,a) += (   -2.00000000) T2(a0,a1,v0,a) W0(w,j,a1,a0,i,v0) 
do s_a0 = 0, nir-1
do s_a1 = 0, nir-1
do s_v0 = 0, nir-1
do s_j = 0, nir-1
...
end subroutine g_sigma_ocov_oovv_no0_x1_type1_eri_c
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_ocov_oovv_no0_x0_type1_eri_o &
  (sa, ia, sa2, ia2, T2, W2, nir, nsym, psym, flops)
...
call g_sigma_ocov_oovv_no0_x0_type1_eri_o &
  (sa, ia, sa2, ia2, av2_i, Xav, d2, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xav)

end subroutine g_if_sigma_ocov_oovv_no0_x0_type1_eri_o
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_ocov_oovv_no0_x0_type1_eri_o &
  (s_a, i_a, s_a2, i_a2, T2_, W2_, D2_, nir, nsym, psym, flops)
...
integer :: s_a1, i_a1, s_a0, i_a0, s_v0, i_v0, s_i, i_i
! W2(i,a2,v0,a) += (    1.00000000) T2(a1,a0,v0,a) D2(i,a1,a2,a0) 
do s_a1 = 0, nir-1
do s_a0 = 0, nir-1
do s_v0 = 0, nir-1
do s_i = 0, nir-1
...
end subroutine g_sigma_ocov_oovv_no0_x0_type1_eri_o
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_ocov_oovv_no0_x1_type1_eri_o &
  (sa, ia, sa2, ia2, V2, W2, S2, nir, nsym, psym, flops)
...
call set_symblock_Xav(sleft, W2, nir, nsym, psym) ! -> Xav (allocate) 
call set_symblock_av2_2(sa, S2, nir, nsym, psym) ! -> av2_i2 (allocate)
call g_sigma_ocov_oovv_no0_x1_type1_eri_o &
  (sa, ia, sa2, ia2, h2_i, Xav, av2_i2, nir, nsym, psym, flops)

deallocate(av2_i2)
deallocate(h2_i)
deallocate(Xav)

end subroutine g_if_sigma_ocov_oovv_no0_x1_type1_eri_o
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_ocov_oovv_no0_x1_type1_eri_o &
  (s_a, i_a, s_a2, i_a2, V2_, W2_, S2_, nir, nsym, psym, flops)
...
integer :: s_w, i_w, s_v0, i_v0, s_j, i_j, s_i, i_i
! S2(i,w,j,a) += (   -2.00000000) V2(a2,w,v0,j) W2(i,a2,v0,a) 
do s_w = 0, nir-1
do s_v0 = 0, nir-1
do s_j = 0, nir-1
do s_i = 0, nir-1
if( &
IEOR(s_i,s_w) == IEOR(s_j,s_a) .and. & 
IEOR(s_a2,s_w) == IEOR(s_v0,s_j) .and. &
...
Z1_(i_w, i_j, i_v0) =  &
  V2_(s_j, s_v0, s_w)%array(i_j, i_v0, i_w)
end do
end do
end do
! Z2 <-- W2(i,a2,v0,a) 
...
end subroutine g_sigma_ocov_oovv_no0_x1_type1_eri_o
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_ocov_oovv_no0_x0_type1_eri_v &
...
call g_sigma_ocov_oovv_no0_x0_type1_eri_v &
  (sa, ia, sv0, iv0, av2_i, Xaa, d2, nir, nsym, psym, flops)

deallocate(av2_i)
deallocate(Xaa)

end subroutine g_if_sigma_ocov_oovv_no0_x0_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_ocov_oovv_no0_x0_type1_eri_v &
...
end subroutine g_sigma_ocov_oovv_no0_x0_type1_eri_v
...
!                                                                       
! **********************************************************************
subroutine g_if_sigma_ocov_oovv_no0_x1_type1_eri_v &
...
call set_symblock_Xaa(sleft, W1, nir, nsym, psym) ! -> Xaa (allocate) 
call set_symblock_av2_2(sa, S2, nir, nsym, psym) ! -> av2_i2 (allocate)
call g_sigma_ocov_oovv_no0_x1_type1_eri_v &
...
end subroutine g_if_sigma_ocov_oovv_no0_x1_type1_eri_v
...
!    -- allRDM.second false
!    -- precedence    -1
subroutine g_sigma_ocov_oovv_no0_x1_type1_eri_v &
...
! S2(i,w,j,a) += (   -2.00000000) V2(v0,a2,w,j) W1(i,a2,a,v0) 
do s_a2 = 0, nir-1
do s_w = 0, nir-1
do s_j = 0, nir-1
do s_i = 0, nir-1
...
end subroutine g_sigma_ocov_oovv_no0_x1_type1_eri_v
