...
#include <sci/icmr/Femto/elems/c_sigma_covv_g.h>                                  
...
orz::mr::BareAmpPack orz::mr::Femto::sigma_covv_g(const orz::mr::Input &ctinp,                                    
...
//-@type(1).contraction(begin)
  ...
  { // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
  // -- No. [    0] -- 
  // |-- [    0] --| S2(w,i,a,b) += (   -1.00000000) T0 V2(b,w,a0,a) D1(i,a0) 
  int sb(s_eri);
  int ib(i_eri);
...
// allocSigma.first = 0
  ...
  FC_FUNC(g_if_sigma_covv_g_no0_x0_type1_eri_v,G_IF_SIGMA_COVV_G_NO0_X0_TYPE1_ERI_V)
    (sb, ib, &T0, V2_sym.cptr(), S2b.cptr(), nir, nsym, psym, &flops);
  ...
  { // <<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
  // -- No. [    1] -- 
  // |-- [    0] --| S2(w,i,a,b) += (    2.00000000) T0 V2(b,a0,w,a) D1(i,a0) 
  int sb(s_eri);
  int ib(i_eri);
  double flops(0); // Flop count  
// |------> 0 (alloc)
// allocSigma.first = 0
  // Pref: 2
  S2b = orz::DTensor(retval.namps_iamp()[ib]);
  FC_FUNC(g_if_sigma_covv_g_no1_x0_type1_eri_v,G_IF_SIGMA_COVV_G_NO1_X0_TYPE1_ERI_V)
    (sb, ib, &T0, V2_sym.cptr(), S2b.cptr(), nir, nsym, psym, &flops);
  retval.acc_amp2(ib, S2b);
  } // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 
...
//-@D4C.contractions(end)
  ...
  // Do timing!
  orz::mr::Femto::my_timer.push_back(boost::make_tuple("sigma_covv_g", time_sigma.elapsed_cputime(), time_sigma.elapsed_wallclocktime()));
